...
OUTPUT_FILENAME = "quick_back.plugin"
SRC_DIR = SCRIPT_DIR / "quick_back"
...
def build():
    ...
    header_path = Path("header.py")
    merge_order = [p for p in merge_order if p != header_path]
    header_raw = HEADER_FILE.read_text(encoding="utf-8").strip()
    ...
    if args.no_minify:
        ...
        if level == "light":
            try:
                ...
                minified_body = ast.unparse(minified_tree)
            except Exception as e:
                print(f"ast.parse light minification failed: {e}")
                minified_body = combined_code
        else:
            ...
            try:
                ...
                minified_body = python_minifier.minify(
                    combined_code,
                ...
                print(f"python_minifier failed: {e}, falling back to ASTMinifier")
                try:
                    tree = ast.parse(combined_code)
                    minifier = ASTMinifier()
                    minified_tree = minifier.visit(tree)
                    ...
                    print(f"fallback ast minification failed: {e2}")
                    minified_body = combined_code
    ...
    try:
        ...
        print(f"Failed to copy to root dist dir: {e}")
